...
``` bash
$ sudo useradd -m k8s
$ echo 'passwd' | sudo passwd k8s --stdin # 设置 k8s 账户密码
...
$ cat /etc/docker/daemon.json # 创建该文件，内容如下：
{
...
$ sudo yum install -y conntrack ipset sysstat curl iptables
```

Ubuntu:

``` bash
$ sudo apt-get install -y conntrack ipset sysstat curl iptables
```
+ ipvs 依赖 ipset；
...
``` bash
$ sudo setenforce 0
$ grep SELINUX /etc/selinux/config  # 修改配置文件，永久生效
SELINUX=disabled
```
...
``` bash
$ sudo mkdir -p /opt/k8s/bin /etc/kubernetes/cert /etc/etcd/cert /var/lib/etcd
$ sudo chown -R k8s /opt/k8s/bin /etc/kubernetes/cert /etc/etcd/cert /var/lib/etcd
```
