...
[Service]
Type=notify
WorkingDirectory=/mnt/disk0/etcd/
ExecStart=/opt/k8s/bin/etcd \\
  --data-dir=/mnt/disk0/etcd \\
  --name=##NODE_NAME## \\
...
```
+ `User`：指定以 k8s 账户运行；
+ `WorkingDirectory`、`--data-dir`：指定工作目录和数据目录为 `/mnt/disk0/etcd`，需在启动服务前创建这个目录；
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "mkdir -p /mnt/disk0/etcd" 
...
``` bash
>>> 172.27.128.150
https://172.27.128.150:2379 is healthy: successfully committed proposal: took = 8.287993ms
>>> 172.27.128.149
https://172.27.128.149:2379 is healthy: successfully committed proposal: took = 21.257989ms
>>> 172.27.128.148
https://172.27.128.148:2379 is healthy: successfully committed proposal: took = 20.712071ms
```
