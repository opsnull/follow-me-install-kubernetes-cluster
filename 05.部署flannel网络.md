...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp /opt/k8s/cert/ca.pem root@${node_ip}:/etc/flanneld/cert
  done
```
...
etcdctl \
  ...
  --cert-file=/opt/k8s/cert/flanneld.pem \
  --key-file=/opt/k8s/cert/flanneld-key.pem \
...
/kubernetes/network/subnets/172.30.55.0-24
...
etcdctl \
  ...
  --cert-file=/opt/k8s/cert/flanneld.pem \
  --key-file=/opt/k8s/cert/flanneld-key.pem \
  get ${FLANNEL_ETCD_PREFIX}/subnets/172.30.24.0-24
```

输出（结果是部署情况而定，网段可能与下面不一样）：

  `{"PublicIP":"172.27.128.71","BackendType":"vxlan","BackendData":{"VtepMAC":"be:1c:19:19:20:2b"}}`
...
>>> 172.27.128.107
    inet 172.30.55.0/32 scope global flannel.1
>>> 172.27.128.123
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    ssh ${node_ip} "ping -c 1 172.30.55.0"
