...
listen kube-master
    ...
    option tcplog
    balance source
    server 172.27.128.71 172.27.128.71:6443 check inter 2000 fall 2 rise 2 weight 1
    server 172.27.128.107 172.27.128.107:6443 check inter 2000 fall 2 rise 2 weight 1
    server 172.27.128.123 172.27.128.123:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ master: 172.27.128.71
+ backup：172.27.128.107、172.27.128.123
...
``` bash
scp keepalived-master.conf root@172.27.128.71:/etc/keepalived/keepalived.conf
```

下发 backup 配置文件：

``` bash
scp keepalived-backup.conf root@172.27.128.107:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@172.27.128.123:/etc/keepalived/keepalived.conf
```
