...
listen kube-master
    ...
    option tcplog
    balance source
    server 172.27.128.120 172.27.128.120:6443 check inter 2000 fall 2 rise 2 weight 1
    server 172.27.128.105 172.27.128.105:6443 check inter 2000 fall 2 rise 2 weight 1
    server 172.27.128.119 172.27.128.119:6443 check inter 2000 fall 2 rise 2 weight 1
EOF
```
...
+ master: 172.27.128.120
+ backup：172.27.128.105、172.27.128.119
...
``` bash
cd /opt/k8s/work
scp keepalived-master.conf root@172.27.128.120:/etc/keepalived/keepalived.conf
```
...
``` bash
cd /opt/k8s/work
scp keepalived-backup.conf root@172.27.128.105:/etc/keepalived/keepalived.conf
scp keepalived-backup.conf root@172.27.128.119:/etc/keepalived/keepalived.conf
```
