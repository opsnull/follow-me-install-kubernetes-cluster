...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "mkdir -p ${K8S_DIR}/kubelet/log" 
...
$ kubectl get csr
NAME                                                   AGE       REQUESTOR                 CONDITION
csr-l4k6q                                              32s       system:node:m7-demo-136002   Approved,Issued
csr-p6854                                              32s       system:node:m7-demo-136003   Approved,Issued
csr-x8mtl                                              33s       system:node:m7-demo-136001   Approved,Issued
...
m7-demo-136001   Ready     <none>    5m        v1.8.15
m7-demo-136002   Ready     <none>    2m        v1.8.15
m7-demo-136003   Ready     <none>    2m        v1.8.15
```
...
tcp        0      0 172.27.136.1:10250     0.0.0.0:*               LISTEN      17692/kubelet
```
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem https://172.27.136.3:10250/metrics # 没有提供证书
Unauthorized

$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer 123456" https://172.27.136.3:10250/metrics # 未授权的 token
Unauthorized
```
...
``` bash
# 权限不足的证书，拒绝访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://172.27.136.3:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

# 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书，授权访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://172.27.136.3:10250/metrics|head
...
echo ${TOKEN}
curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://172.27.136.3:10250/metrics | head
```
...
浏览器访问 https://172.27.136.1:10250/metrics 和 https://172.27.136.1:10250/metrics/cadvisor 分别返回 kublet 和 cadvisor 的 metrics。
