...
[Service]
WorkingDirectory=/mnt/disk1/k8s/kubelet
ExecStart=/opt/k8s/bin/kubelet \\
  --root-dir=/mnt/disk1/k8s/kubelet \\
  --address=##NODE_IP## \\
  --port=10250 \\
  --read-only-port=0 \\
  ...
  --log-dir=/mnt/disk1/k8s/kubelet/log \\
  --v=2
Restart=on-failure
RestartSec=5
...
+ `--root-dir=/mnt/disk1/k8s/kubelet`：kublet 数据目录；
...
+ `--event-qps=0`、`--kube-api-qps=2000`、`--registry-qps=20`、`--sync-frequency=2m`：调大 qps 和最大同步频率，否则节点 Pod 数很多时(200+)，Pod 状态更新非常缓慢(小时级别)，导致服务长时间不可用；
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "mkdir -p /mnt/disk1/k8s/kubelet/log" 
...
$ kubectl get csr
NAME                                                   AGE       REQUESTOR                 CONDITION
csr-l4k6q                                              32s       system:node:m7-demo-128105   Approved,Issued
csr-p6854                                              32s       system:node:m7-demo-128119   Approved,Issued
csr-x8mtl                                              33s       system:node:m7-demo-128120   Approved,Issued
node-csr-D0F3jr01NOrVXm9-kAL_K_IKww43-OdlY7N_BI6eAmA   1m        kubelet-bootstrap             Approved,Issued
node-csr-ZTgRNjF0vOrIUB2ZAnBJLaEQV_YUQkGPbLp16GvZ0pE   1m        kubelet-bootstrap             Approved,Issued
node-csr-uNDpD92EvQkoB3171q-wZzjVhMw76NyhEDuq2M0_3MA   1m        kubelet-bootstrap             Approved,Issued
```
...
m7-demo-128120   Ready     <none>    5m        v1.8.15
m7-demo-128105   Ready     <none>    2m        v1.8.15
m7-demo-128119   Ready     <none>    2m        v1.8.15
```
...
-rw------- 1 root root 2295 8月  15 22:34 /etc/kubernetes/kubelet.kubeconfig

$ ls -l /etc/kubernetes/cert/|grep kubelet
-rw-r--r-- 1 root root 1054 8月  15 22:34 kubelet-client.crt
-rw------- 1 root root  227 8月  15 22:33 kubelet-client.key
-rw------- 1 root root 1354 8月  15 22:34 kubelet-server-2018-08-15-22-34-02.pem
lrwxrwxrwx 1 root root   59 8月  15 22:34 kubelet-server-current.pem -> /etc/kubernetes/cert/kubelet-server-2018-08-15-22-34-02.pem
```
...
tcp        0      0 172.27.128.120:10250     0.0.0.0:*               LISTEN      17692/kubelet
```
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem https://172.27.128.119:10250/metrics # 没有提供证书
Unauthorized

$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer 123456" https://172.27.128.119:10250/metrics # 未授权的 token
Unauthorized
```
...
``` bash
# 权限不足的证书，拒绝访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://172.27.128.119:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

# 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书，授权访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://172.27.128.119:10250/metrics|head
...
echo ${TOKEN}
curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://172.27.128.119:10250/metrics|head
```
...
浏览器访问 https://172.27.128.120:10250/metrics 和 https://172.27.128.120:10250/metrics/cadvisor 分别返回 kublet 和 cadvisor 的 metrics。
