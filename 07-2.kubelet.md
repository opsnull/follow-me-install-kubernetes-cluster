...
+ `--root-dir=/var/lib/kubelet`：kublet 数据目录；
...
-rw------- 1 root root 1273 7月   4 12:47 /etc/kubernetes/cert/kubelet-client-2018-07-04-12-47-15.pem
-rw-r--r-- 1 root root 1046 7月   4 12:02 /etc/kubernetes/cert/kubelet-client.crt
lrwxrwxrwx 1 root root   59 7月   4 12:47 /etc/kubernetes/cert/kubelet-client-current.pem -> /etc/kubernetes/cert/kubelet-client-2018-07-04-12-47-15.pem
-rw------- 1 root root  227 7月   4 12:02 /etc/kubernetes/cert/kubelet-client.key
-rw------- 1 root root 1334 7月   4 12:46 /etc/kubernetes/cert/kubelet-server-2018-07-04-12-46-10.pem
lrwxrwxrwx 1 root root   59 7月   4 12:46 /etc/kubernetes/cert/kubelet-server-current.pem -> /etc/kubernetes/cert/kubelet-server-2018-07-04-12-46-10.pem
```
...
tcp        0      0 172.27.132.67:10150     0.0.0.0:*               LISTEN      6619/kubelet
tcp        0      0 127.0.0.1:10248         0.0.0.0:*               LISTEN      6619/kubelet
```
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem https://172.27.132.67:10250/metrics # 没有提供证书
Unauthorized

$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer 123456" https://172.27.132.67:10250/metrics # 未授权的 token
Unauthorized
```
...
``` bash
# 权限不足的证书，拒绝访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://172.27.132.67:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

# 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书，授权访问；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert admin.pem --key admin-key.pem https://172.27.132.67:10250/metrics|head
...
`06-1.api-server.md`一文中定义的 ClusterRole system:kubelet-api-admin 授予访问 kubelet 所有 API 的权限。
...
echo ${TOKEN}
curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://172.27.132.67:10250/metrics|head
```
...
cadvisor 统计所在节点各容器的资源(CPU、内存、磁盘、网卡)使用情况，在 10250 以 promehteus metrics 的形式输出。
