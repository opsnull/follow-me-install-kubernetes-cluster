...
``` bash
sudo mount -t ceph 172.27.128.100:6789:/k8s/  /mnt/cephfs/k8s/devops -o name=admin,secretfile=/etc/ceph/ceph-admin.secret,noatime
```
...
[root@m7-demo-128120 k8s]# cd /opt/k8s
[root@m7-demo-128120 k8s]# scp root@172.27.128.100:/etc/ceph/ceph.client.admin.keyring /etc/ceph/

[root@m7-demo-128120 k8s]# cat /etc/ceph/ceph.client.admin.keyring
[client.admin]
        key = AQCYLTdbCyxZBhAAbGfK3T2tczjbhhbR0UWq1w==

[root@m7-demo-128120 k8s]# Secret=$(awk '/key = / {print $3}' /etc/ceph/ceph.client.admin.keyring | base64)
[root@m7-demo-128120 k8s]# cat > ceph-secret-admin.yaml <<EOF
apiVersion: v1
kind: Secret
...
[root@m7-demo-128120 k8s]#  kubectl create -f ceph-secret-admin.yaml
...
``` bash
[root@m7-demo-128120 k8s]# cat >ceph-rbd-storage-class.yaml <<EOF
...
``` bash
[root@m7-demo-128120 k8s]# kubectl create -f ceph-rbd-storage-class.yaml
...
[root@m7-demo-128120 k8s]# cat >ceph-pvc-storageClass.json <<EOF
{
...
[root@m7-demo-128120 k8s]# kubectl create -f ceph-pvc-storageClass.json
persistentvolumeclaim "pvc-test-claim" created

[root@m7-demo-128120 k8s]# kubectl get pv
NAME                                       CAPACITY   ACCESS MODES   RECLAIM POLICY   STATUS    CLAIM                    STORAGECLASS   REASON    AGE
pvc-0335828d-90b7-11e8-b43c-0cc47a2af650   1Gi        RWO            Delete           Bound     default/pvc-test-claim   ceph                     20m
[root@m7-demo-128120 k8s]# kubectl describe pv pvc-0335828d-90b7-11e8-b43c-0cc47a2af650
...
[root@m7-demo-128120 k8s]# cat > ceph-pv-test.yaml <<EOF
apiVersion: v1
kind: Pod
metadata:
  name: ceph-pv-test
...
[root@m7-demo-128120 k8s]# kubectl create -f ceph-pv-test.yaml
pod "ceph-pv-test" created

[root@m7-demo-128120 k8s]# kubectl describe pods ceph-pv-test
Name:         ceph-pv-test
Namespace:    default
Node:         m7-demo-128120/172.27.128.120
...
Events:
  ...
  ----    ------                 ----               ----                       -------
  Normal  SuccessfulMountVolume  51s                kubelet, m7-demo-128120  MountVolume.SetUp succeeded for volume "default-token-4km88"
  Normal  Scheduled              50s                default-scheduler          Successfully assigned ceph-pv-test to m7-demo-128120
  Normal  SuccessfulMountVolume  50s (x2 over 50s)  kubelet, m7-demo-128120  MountVolume.SetUp succeeded for volume "pvc-0335828d-90b7-11e8-b43c-0cc47a2af650"
  Normal  Pulling                49s                kubelet, m7-demo-128120  pulling image "busybox"
  Normal  Pulled                 46s                kubelet, m7-demo-128120  Successfully pulled image "busybox"
  Normal  Created                46s                kubelet, m7-demo-128120  Created container
  Normal  Started                46s                kubelet, m7-demo-128120  Started container
